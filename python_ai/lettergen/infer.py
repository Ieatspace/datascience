...
DEFAULT_CONFIG_PATH = PROJECT_ROOT / "out" / "letter_gen_config.json"
...
def _postprocess_alpha(mask: np.ndarray, style_strength: float) -> np.ndarray:
    ...
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats((alpha > 0).astype(np.uint8), connectivity=8)
    if num_labels > 2:
        areas = stats[:, cv2.CC_STAT_AREA]
        keep_idx = int(np.argmax(areas[1:]) + 1) if len(areas) > 1 else 0
        if keep_idx > 0:
            alpha = np.where(labels == keep_idx, 255, 0).astype(np.uint8)
    return alpha
...
def load_model(weights_path: Optional[Path] = None, device: Optional[str] = None) -> LetterGeneratorRuntime:
    """Public cache-backed loader required by the renderer and tooling."""
    return load_generator(weights_path=weights_path, device=device)
...
def generate_letter(
    ...
    """Generate a new RGBA glyph image from the learned latent distribution.

    The returned image is *synthesized* by the decoder from a sampled latent vector,
    not copied from `out/chars`.
    """
    strength = float(style_strength if temperature is None else temperature)
    ...
    return Image.fromarray(glyph.rgba, mode="RGBA")
