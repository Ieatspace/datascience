...
def main() -> None:
    p = argparse.ArgumentParser(description="Sample learned glyphs from the cVAE letter generator")
    p.add_argument("--weights", type=Path, default=DEFAULT_WEIGHTS_PATH)
    p.add_argument("--letter", default="a", help="'a'..'z' or 'all'")
    p.add_argument("--n", "--num", type=int, default=16, dest="num")
    ...
    if args.letter.lower() == "all":
        ...
        out_path = args.out or (DEFAULT_OUT_DIR / "samples_all_letters.png")
    ...
    out_path = args.out or (DEFAULT_OUT_DIR / "samples_{0}.png".format(letter))
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if not cv2.imwrite(str(out_path), grid):
        raise SystemExit("Failed to write sample grid: {0}".format(out_path))
    print("[sample] saved {0}".format(out_path))
