...
class GlyphRecord:
    ...
    image: np.ndarray  # float32 [H, W] in [0, 1], centered/padded model input
...
def make_weighted_sampler(dataset: "GlyphDataset", records: Sequence[GlyphRecord]):
    """Optional class-balancing sampler for small/imbalanced datasets."""
    ...
    for idx in dataset.indices:
        label_idx = int(records[int(idx)].label_idx)
        counts[label_idx] = counts.get(label_idx, 0) + 1

    weights = []
    for idx in dataset.indices:
        label_idx = int(records[int(idx)].label_idx)
        count = max(1, counts.get(label_idx, 1))
